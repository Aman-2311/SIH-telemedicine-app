...
export const AuthScreen: React.FC<AuthScreenProps> = ({
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    if (isCreating) {
      ...
      const ok = await login({ abha_id: `${selectedRole.toUpperCase()}-MH-${Math.floor(1000 + Math.random() * 9000)}`, role: selectedRole });
      if (ok) {
        setStep("success");
        setTimeout(() => onSuccessRole?.(selectedRole), 800);
      }
        ...
        <div className="auth-card fade-in">
          ...
          <form onSubmit={handleSendOtp} className="auth-form-inner">
            ...
            <div className="auth-demo-note">
              ...
              <span>Demo: Pre-filled number works instantly</span>
            </div>
