    ...
    try {
      ...
      if (latestRxRecord && latestRxRecord.prescription && latestRxRecord.prescription.diagnosis) {
        ...
        setActivePrescription({
          ...
          doctor_name: rx.doctor_name || "Dr. Arvind Kulkarni (MD)",
          ...
          doctor_advice: rx.notes || "Follow prescribed dosage and take medicines with water after food.",
        });
      } else {
          ...
          {activeTab === "prescriptions" && (
            <main className="patient-primary-pane wide">
              ...
              <div className="patient-pill-tabs">
                ...
                >
                  Active ({activePrescription ? 1 : 0})
                </button>
                <button
                  className={`patient-pill-tab ${rxTab === "past" ? "active" : ""}`}
                  onClick={() => setRxTab("past")}
                >
                  Consultation History ({patientHistory.length})
                </button>
              </div>
                ...
                {rxTab === "active" ? (
                  activePrescription ? (
                    <div className="lg:col-span-2 patient-rx-card">
                      ...
                      {/* Medicines List with Jan Aushadhi Savings */}
                      <div className="mb-6">
                        ...
                        {activePrescription.medicines.map((med: any, idx: number) => (
                          <div key={idx} className="patient-medicine-item">
                      ...
                      <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6">
                        ...
                        <p className="text-sm text-amber-950">{activePrescription.doctor_advice}</p>
                      </div>
                    ...
                    <div className="lg:col-span-2 bg-white rounded-2xl p-8 border border-slate-200 text-center flex flex-col items-center justify-center min-h-[300px]">
                      ...
                      <p className="text-xs text-slate-500 max-w-sm mx-auto mb-4">
                        {patientHistory.length > 0
                          ? "Your consultation intake has been recorded and is waiting for review by the consulting doctor."
                          : "No active prescriptions or pending consultations on record."}
                      </p>
                      {patientHistory.length > 0 && (
                        <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-amber-50 text-amber-800 text-xs font-semibold border border-amber-200">
                          <span className="w-2 h-2 rounded-full bg-amber-500 animate-pulse" />
                          Status: {patientHistory[0].status === "waiting" ? "Queued for Doctor Review" : patientHistory[0].status}
                        </div>
                      )}
                    </div>
                  )
                        ...
                        <div key={idx} className="bg-white rounded-xl p-4 border border-slate-200 shadow-sm">
                          ...
                          {item.prescription?.medicines && item.prescription.medicines.length > 0 && (
                            <div className="text-xs text-slate-600 mt-2 pt-2 border-t border-slate-100">
                              ...
                              {item.prescription.medicines.map((m: any) => m.name).join(", ")}
                            </div>
                          )}
                        </div>
                      ))
